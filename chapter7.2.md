...
7.2.4.2.7SDO分段上传服务
客户端通过SDO分段上传服务向服务器请求下一个数据分段。接下来的参数向客户端表明是否还有更多分段或此为最后一个上传分段，SDO分段上传服务至少应包含一个分段。用于此服务的参数被定义在表 12中。
